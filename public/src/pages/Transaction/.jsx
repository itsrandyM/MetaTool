...
const AddToken = ({ isOpen, onClose, onSubmit }) => {
  ...
  const tokenOptions = ["USDT", "ADA", "DJED", "BITCOIN", ""]; // Add your token options here

  // Convert tokenOptions to select format
  const selectOptions = tokenOptions.map(option => ({
    value: option,
    label: option
  }));

  // Handler for selecting a token
  const handleTokenSelect = (selectedOption, index) => {
    const updatedTokens = [...tokens];
        ...
        {tokens.map((token, index) => (
          <div key={index} style={{ marginBottom: '10px' }}>
            <Select
              ...
              value={{ value: token.name, label: token.name }}
              onChange={(selectedOption) => handleTokenSelect(selectedOption, index)}
